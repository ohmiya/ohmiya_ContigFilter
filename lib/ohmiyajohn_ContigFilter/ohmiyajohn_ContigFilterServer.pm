package ohmiyajohn_ContigFilter::ohmiyajohn_ContigFilterServer;
...
our %async_run_methods = (
        'filter_contigs_async' => 'ohmiyajohn_ContigFilter.filter_contigs',
);

our %async_check_methods = (
        'filter_contigs_check' => 'ohmiyajohn_ContigFilter.filter_contigs',
);
...
sub get_service_name
{
    ...
    if(!defined $ENV{$SERVICE}) {
        return 'ohmiyajohn_ContigFilter';
    }
    return $ENV{$SERVICE};
}
...
sub call_method {
    ...
    my $ctx = ohmiyajohn_ContigFilter::ohmiyajohn_ContigFilterServerContext->new($self->{loggers}->{userlog},
...
{
    # Service ohmiyajohn_ContigFilter requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for ohmiyajohn_ContigFilter but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = ohmiyajohn_ContigFilter::ohmiyajohn_ContigFilterServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service ohmiyajohn_ContigFilter.");
    }
...
package ohmiyajohn_ContigFilter::ohmiyajohn_ContigFilterServerContext;

use strict;

=head1 NAME

ohmiyajohn_ContigFilter::ohmiyajohn_ContigFilterServerContext
...
package ohmiyajohn_ContigFilter::ohmiyajohn_ContigFilterServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = ohmiyajohn_ContigFilter::ohmiyajohn_ContigFilterServer->new(
